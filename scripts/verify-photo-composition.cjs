...
app.whenReady().then(async () => {
    ...
    const win = new BrowserWindow({ show:false, width:315, height:400, webPreferences:{ offscreen:true } });
    await win.loadURL('about:blank');
    const metrics = await win.webContents.executeJavaScript(`(async () => {
        const STUDENT_PHOTO_WIDTH = 315;
        const STUDENT_PHOTO_HEIGHT = 400;
        const STUDENT_PHOTO_BLUE = '#3b86ee';
        ${composer}
        ...
        const photo = document.createElement('canvas'); photo.width = 315; photo.height = 400;
        photo.getContext('2d').drawImage(output, 0, 0);
        document.body.style.margin = '0'; document.body.append(photo);
        const context = photo.getContext('2d');
        const top = context.getImageData(157, 12, 1, 1).data;
        const bottom = context.getImageData(157, 392, 1, 1).data;
        const blueAbove = top[2] > top[1] + 35;
        const shirtAtBottom = bottom[0] > 185 && bottom[1] > 185 && bottom[2] > 185;
        return { blueAbove, shirtAtBottom, jpegBytes:output.src.length, width:output.width, height:output.height, preview:photo.toDataURL('image/png') };
    })()`);
    ...
    win.destroy();
    if (!metrics.blueAbove || !metrics.shirtAtBottom || metrics.jpegBytes > 49000 || metrics.width !== 315 || metrics.height !== 400) {
